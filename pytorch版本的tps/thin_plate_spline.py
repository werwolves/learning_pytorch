...
class ThinPlateSpline:
    ...
    def fit(self, X: torch.Tensor, Y: torch.Tensor) -> ThinPlateSpline:
        ...
        n_c, d_s = X.shape  # [14, 2]
        self.control_points = X  # [[ 37.3504,  92.5478],[ 95.0800,  69.6177],...]

        phi = self._radial_distance(X)   # phi.shape=[14, 14]

        # Build the linear system AP = Y
        X_p = torch.hstack([torch.ones((n_c, 1), device=self.device), X])  # [[1, 37.3504,  92.5478],[1,  95.0800,  69.6177],...]

        A = torch.vstack(
            [
                torch.hstack([phi + self.alpha * torch.eye(n_c, device=self.device), X_p]), # .shape=[14, 17]
                torch.hstack([X_p.T, torch.zeros((d_s + 1, d_s + 1), device=self.device)]), # .shape=[3,17]
            ]
        )   # ====> .shape=[17,17]
